...
> 这个仓库是我从 [bingreeky/MemEvolve](https://github.com/bingreeky/MemEvolve) fork 来的，用于「前沿 AI 方向在线考核 · 题目一」。我的改动都以独立 commit/PR 叠加在原实现之上，方便和原作者的代码区分。

我在 Flash-Searcher 上用 `deepseek-v4-flash`，在 xBench-DeepSearch 前 20 条任务上对比了 4 个设置（每个 memory run 前清空记忆、串行积累）：
...
主要发现：强基线下 memory 的准确率收益约等于 0，成本却显著上升；收益（问题框架固定、策略迁移）和伤害（未验证的中间结论被记忆"钉死"）集中在难题上互相抵消；Voyager 的技能抽取和深搜任务结构性不匹配、静默失效；我还发现并修复了 xBench 判分统计恒为 0% 的 harness bug，并给 lightweight 的 working memory 实现了一个"验证门控" patch 做了 ablation。

完整报告（设置 / 结果 / case 分析 / limitation / 改进与 ablation）见 [REPORT.md](REPORT.md)。复现命令在报告 §1，结果汇总脚本是 `Flash-Searcher-main/summarize_results.py`。
